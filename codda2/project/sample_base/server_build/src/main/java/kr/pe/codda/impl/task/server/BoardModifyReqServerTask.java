...
public class BoardModifyReqServerTask extends AbstractServerTask
	...
	private Logger log = LoggerFactory.getLogger(BoardModifyReqServerTask.class);
	...
	public void doTask(String projectName, LoginManagerIF personalLoginManager, ToLetterCarrier toLetterCarrier,
		...
		AbstractMessage outputMessage = ServerDBUtil.execute(
				ServerCommonStaticFinalVars.DEFAULT_DBCP_NAME, this, (BoardModifyReq) inputMessage);
	...
	public BoardModifyRes doWork(final DSLContext dsl, final BoardModifyReq boardModifyReq) throws Exception {
		...
		if (null == boardModifyReq) {
			throw new ParameterServerTaskException("the parameter boardModifyReq is null");
		}
		...
		try {
			...
			ValueChecker.checkValidAttachedFilCount(boardModifyReq.getOldAttachedFileCnt(),
		...
		final int newNextAttachedFileSeq = boardModifyReq.getNextAttachedFileSeq()
				+ boardModifyReq.getNewAttachedFileCnt();
		...
		MemberRoleType memberRoleTypeOfRequestedUserID = ServerDBUtil.checkUserAccessRights(dsl, "게시판 수정 서비스",
				PermissionType.GUEST, boardModifyReq.getRequestedUserID());
		...
		int boardHistoryInsertCount = dsl.insertInto(SB_BOARD_HISTORY_TB).set(SB_BOARD_HISTORY_TB.BOARD_ID, boardID)
				.set(SB_BOARD_HISTORY_TB.BOARD_NO, boardNo).set(SB_BOARD_HISTORY_TB.HISTORY_SQ, newBoardHistorySeq)
				.set(SB_BOARD_HISTORY_TB.SUBJECT,
						(BoardListType.ONLY_GROUP_ROOT.equals(boardListType) && (parenetNo.longValue() != 0L) ? null
		...
		if (attachFileListRecord.isEmpty()) {
			...
			if (!deletedAttachedFileSequeceSet.containsAll(remainingOldAttachedFileSequeceSet)) {
				...
				String errorMessage = new StringBuilder("보존을 원하는 구 첨부 파일 목록[=")
						...
						.append(deletedAttachedFileSequeceSet.toString()).append("]에 존재하지 않는 첨부 파일이 존재합니다").toString();
				throw new RollbackServerTaskException(errorMessage);
			}
