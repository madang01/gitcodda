...
public class PersonalActivityHistoryReqServerTask extends AbstractServerTask
	...
	public void doTask(String projectName, LoginManagerIF personalLoginManager, ToLetterCarrier toLetterCarrier,
		...
		AbstractMessage outputMessage = ServerDBUtil.execute(ServerCommonStaticFinalVars.DEFAULT_DBCP_NAME, this,
				(PersonalActivityHistoryReq) inputMessage);
	...
	@Override
	public PersonalActivityHistoryRes doWork(final DSLContext dsl,
			final PersonalActivityHistoryReq personalActivityHistoryReq) throws Exception {
		if (null == dsl) {
			throw new ParameterServerTaskException("the parameter dsl is null");
		}

		if (null == personalActivityHistoryReq) {
			throw new ParameterServerTaskException("the parameter personalActivityHistoryReq is null");
		}
		...
		try {
			...
			ValueChecker.checkValidPageNoAndPageSize(personalActivityHistoryReq.getPageNo(),
					personalActivityHistoryReq.getPageSize());
		...
		final List<PersonalActivityHistoryRes.PersonalActivity> personalActivityList = new ArrayList<PersonalActivityHistoryRes.PersonalActivity>();

		final PersonalActivityHistoryRes personalActivityHistoryRes = new PersonalActivityHistoryRes();

		MemberRoleType memberRoleTypeOfRequestedUserID = ServerDBUtil.checkUserAccessRights(dsl, "개인 활동 내역 조회 서비스",
				PermissionType.GUEST, personalActivityHistoryReq.getRequestedUserID());

		Record3<String, Byte, Byte> targetUserMemberRecord = dsl
				.select(SB_MEMBER_TB.NICKNAME, SB_MEMBER_TB.ROLE, SB_MEMBER_TB.STATE).from(SB_MEMBER_TB)
		...
		try {
			...
			String errorMessage = new StringBuilder("개인 활동 내역 대상 사용자[")
					.append(personalActivityHistoryReq.getTargetUserID()).append("]의 멤버 구분[")
					.append(targetUserMemberRole).append("]이 잘못되었습니다").toString();
		...
		try {
			...
			String errorMessage = new StringBuilder("개인 활동 내역 대상 사용자[")
					.append(personalActivityHistoryReq.getTargetUserID()).append("]의 멤버 상태[")
					.append(targetUserMemberState).append("]가 잘못되었습니다").toString();
			throw new RollbackServerTaskException(errorMessage);
		}
		...
		if (!MemberStateType.OK.equals(targetUserMemberStateType)
			...
			String errorMessage = new StringBuilder("개인 활동 내역 대상 사용자[")
					.append(personalActivityHistoryReq.getTargetUserID()).append("] 상태[")
					.append(targetUserMemberStateType.getName()).append("]가 정상이 아닙니다").toString();
			throw new RollbackServerTaskException(errorMessage);
		}

		Record1<Long> totalRecord = dsl
				.select(DSL.field("if ({0} is null, {1}, {2})", Long.class,
		...
		if (total > 0) {
			...
			Table<Record9<UInteger, Byte, UByte, UInteger, Timestamp, Byte, String, Byte, UInteger>> mainTable = dsl
					.select(SB_MEMBER_ACTIVITY_HISTORY_TB.ACTIVITY_SQ, SB_MEMBER_ACTIVITY_HISTORY_TB.ACTIVITY_TYPE,
							SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_ID, SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_NO,
							SB_MEMBER_ACTIVITY_HISTORY_TB.REG_DT,
							dsl.select(SB_BOARD_INFO_TB.LIST_TYPE).from(SB_BOARD_INFO_TB)
									.where(SB_BOARD_INFO_TB.BOARD_ID.eq(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_ID))
									.<Byte>asField(SB_BOARD_INFO_TB.LIST_TYPE.getName()),
							dsl.select(SB_BOARD_INFO_TB.BOARD_NAME).from(SB_BOARD_INFO_TB)
									.where(SB_BOARD_INFO_TB.BOARD_ID.eq(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_ID))
									.<String>asField(SB_BOARD_INFO_TB.BOARD_NAME.getName()),
							SB_BOARD_TB.BOARD_ST, SB_BOARD_TB.GROUP_NO)
					.from(SB_MEMBER_ACTIVITY_HISTORY_TB).innerJoin(SB_BOARD_TB)
					...
					.where(SB_MEMBER_ACTIVITY_HISTORY_TB.USER_ID.eq(personalActivityHistoryReq.getTargetUserID()))
					.and(SB_MEMBER_ACTIVITY_HISTORY_TB.ACTIVITY_SQ.ge(UInteger.valueOf(startActivitySeq)))
					.and(SB_MEMBER_ACTIVITY_HISTORY_TB.ACTIVITY_SQ.lt(UInteger.valueOf(endActivitySeq))).asTable("a");
			...
			Result<Record11<Byte, UByte, UInteger, Timestamp, Byte, String, Byte, UInteger, String, String, String>> memberActivityHisotryResult = dsl
					.select(mainTable.field(SB_MEMBER_ACTIVITY_HISTORY_TB.ACTIVITY_TYPE),
							...
							mainTable.field(SB_BOARD_INFO_TB.LIST_TYPE), mainTable.field(SB_BOARD_INFO_TB.BOARD_NAME),
							mainTable.field(SB_BOARD_TB.BOARD_ST), mainTable.field(SB_BOARD_TB.GROUP_NO),
							DSL.choose(mainTable.field(SB_BOARD_INFO_TB.LIST_TYPE))
									.when(BoardListType.TREE.getValue(), d.SUBJECT).otherwise(b.SUBJECT)
									.as("sourceSubject"),
							DSL.choose(mainTable.field(SB_BOARD_INFO_TB.LIST_TYPE))
									.when(BoardListType.TREE.getValue(), personalActivityHistoryReq.getTargetUserID())
									.otherwise(c.REGISTRANT_ID).as("sourceWriterID"),
							DSL.choose(mainTable.field(SB_BOARD_INFO_TB.LIST_TYPE))
									.when(BoardListType.TREE.getValue(), targetUserNickname)
									.otherwise(dsl.select(SB_MEMBER_TB.NICKNAME).from(SB_MEMBER_TB)
											.where(SB_MEMBER_TB.USER_ID.eq(c.REGISTRANT_ID)).<String>asField())
									.as("sourceWriterNickname"))
					.from(mainTable).innerJoin(b)
					...
					.and(b.field(SB_BOARD_HISTORY_TB.HISTORY_SQ)
							.eq(dsl.select(SB_BOARD_HISTORY_TB.HISTORY_SQ.max()).from(SB_BOARD_HISTORY_TB)
									.where(SB_BOARD_HISTORY_TB.BOARD_ID
											.eq(mainTable.field(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_ID)))
									.and(SB_BOARD_HISTORY_TB.BOARD_NO.eq(mainTable.field(SB_BOARD_TB.GROUP_NO)))))
					.innerJoin(c).on(c.BOARD_ID.eq(mainTable.field(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_ID)))
					.and(c.BOARD_NO.eq(mainTable.field(SB_BOARD_TB.GROUP_NO)))
					.and(c.field(SB_BOARD_HISTORY_TB.HISTORY_SQ).eq(UByte.valueOf(0))).innerJoin(d)
					...
					.and(d.field(SB_BOARD_HISTORY_TB.HISTORY_SQ)
							.eq(dsl.select(SB_BOARD_HISTORY_TB.HISTORY_SQ.max()).from(SB_BOARD_HISTORY_TB)
									.where(SB_BOARD_HISTORY_TB.BOARD_ID
											.eq(mainTable.field(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_ID)))
									.and(SB_BOARD_HISTORY_TB.BOARD_NO
											.eq(mainTable.field(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_NO)))))
					.orderBy(mainTable.field(SB_MEMBER_ACTIVITY_HISTORY_TB.ACTIVITY_SQ).desc()).fetch();
			...
			for (Record11<Byte, UByte, UInteger, Timestamp, Byte, String, Byte, UInteger, String, String, String> memberActivityHisotryRecord : memberActivityHisotryResult) {
				byte memberActivityTypeValue = memberActivityHisotryRecord
						.get(SB_MEMBER_ACTIVITY_HISTORY_TB.ACTIVITY_TYPE);
				UByte boardIDOfActivityHistory = memberActivityHisotryRecord
						.get(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_ID);
				UInteger boardNoOfActivityHistory = memberActivityHisotryRecord
						.get(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_NO);
