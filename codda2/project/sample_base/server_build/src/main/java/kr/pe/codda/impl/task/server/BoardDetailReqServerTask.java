...
public class BoardDetailReqServerTask extends AbstractServerTask
		implements DBAutoCommitTaskIF<BoardDetailReq, BoardDetailRes> {
	private Logger log = LoggerFactory.getLogger(BoardDetailReqServerTask.class);
	...
	public void doTask(String projectName, LoginManagerIF personalLoginManager, ToLetterCarrier toLetterCarrier,
		...
		AbstractMessage outputMessage = ServerDBUtil.execute(
				ServerCommonStaticFinalVars.DEFAULT_DBCP_NAME, this, (BoardDetailReq) inputMessage);
		toLetterCarrier.addSyncOutputMessage(outputMessage);
	}
	
	public BoardDetailRes doWork(final String dbcpName, final BoardDetailReq boardDetailReq) throws Exception {
		BoardDetailRes outputMessage = ServerDBUtil.execute(
				dbcpName, this, boardDetailReq);
		
		return outputMessage;
	}

	public BoardDetailRes doWork(final DSLContext dsl, final BoardDetailReq boardDetailReq) throws Exception {
		if (null == dsl) {
			throw new ParameterServerTaskException("the parameter dsl is null");
		}

		if (null == boardDetailReq) {
			throw new ParameterServerTaskException("the parameter boardDetailReq is null");
		}
		...
		MemberRoleType memberRoleTypeOfRequestedUserID = ServerDBUtil.checkUserAccessRights(dsl, "게시글 상세 조회 서비스",
				PermissionType.GUEST, boardDetailReq.getRequestedUserID());
		...
		Record18<UInteger, UShort, UInteger, UByte, Integer, Byte, UByte, String, Object, String, String, Timestamp, String, Object, Timestamp, String, Object, Object> mainBoardRecord = dsl
				.select(SB_BOARD_TB.GROUP_NO, SB_BOARD_TB.GROUP_SQ, SB_BOARD_TB.PARENT_NO, SB_BOARD_TB.DEPTH,
						...
						dsl.select(SB_MEMBER_TB.ROLE).from(SB_MEMBER_TB).where(SB_MEMBER_TB.USER_ID.eq(c.REGISTRANT_ID))
		...
		try {
			...
			String errorMessage = new StringBuilder("게시글의 DB 상태 값[").append(boardState).append("]이 잘못되었습니다").toString();
			throw new RollbackServerTaskException(errorMessage);
		}
		...
		if (BoardListType.ONLY_GROUP_ROOT.equals(boardListType)) {
			...
			if (0L != parentNo.longValue()) {
				String errorMessage = new StringBuilder().append(boardName).append(" 게시판[").append(boardID)
						.append("]은 본문로만 이루어진 목록을 갖는 게시판으로 본문만 상세 조회가 가능합니다. 상세 조회를 요청한 게시글[boardNo=").append(boardNo)
						.append("]은 본문이 아닙니다").toString();
				throw new RollbackServerTaskException(errorMessage);
			}
			...
			Table<Record3<UByte, UInteger, UShort>> d = dsl.select(a.BOARD_ID, a.GROUP_NO, a.GROUP_SQ)
					...
					.and(a.BOARD_ST.eq(BoardStateType.OK.getValue())).and(a.GROUP_NO.eq(groupNo))
			...
			Table<Record10<UByte, UInteger, UInteger, UShort, UInteger, UByte, Integer, Byte, UByte, String>> mainTable = dsl
					...
					.and(a.GROUP_NO.eq(d.field(SB_BOARD_TB.GROUP_NO))).and(a.GROUP_SQ.eq(d.field(SB_BOARD_TB.GROUP_SQ)))
					.asTable("a");
			...
			Result<Record16<UInteger, UShort, UInteger, UByte, Byte, UByte, String, Object, String, Timestamp, String, Object, Timestamp, String, Object, Object>> childBoardResult = dsl
					.select(mainTable.field(SB_BOARD_TB.BOARD_NO), mainTable.field(SB_BOARD_TB.GROUP_SQ),
							...
							mainTable.field(SB_BOARD_TB.BOARD_ST), mainTable.field(SB_BOARD_TB.NEXT_ATTACHED_FILE_SQ),
					...
					.and(b.HISTORY_SQ.eq(dsl.select(b.HISTORY_SQ.max()).from(b)
							...
							.and(b.BOARD_NO.eq(mainTable.field(SB_BOARD_TB.BOARD_NO)))))
					.orderBy(mainTable.field(SB_BOARD_TB.GROUP_SQ).desc()).fetch();
		...
		int countOfViewCountUpdate = dsl.update(SB_BOARD_TB).set(SB_BOARD_TB.VIEW_CNT, SB_BOARD_TB.VIEW_CNT.add(1))
