...
public class BoardMoveReqServerTask extends AbstractServerTask
	...
	public BoardMoveRes doWork(final DSLContext dsl, final BoardMoveReq boardMoveReq) throws Exception {
		...
		if (null == boardMoveReq) {
			throw new ParameterServerTaskException("the parameter boardMoveReq is null");
		}
		...
		MemberRoleType memberRoleTypeOfRequestedUserID = ServerDBUtil.checkUserAccessRights(dsl, "게시글 이동 서비스",
				PermissionType.ADMIN, boardMoveReq.getRequestedUserID());
		...
		Record4<Byte, Byte, Byte, Byte> sourceBoardInforRecord = dsl
				...
				.from(SB_BOARD_INFO_TB).where(SB_BOARD_INFO_TB.BOARD_ID.eq(sourceBoardID)).fetchOne();
		...
		try {
			...
			String errorMessage = new StringBuilder().append("이동할  게시글의 ").append(e.getMessage()).toString();
			throw new RollbackServerTaskException(errorMessage);
		}
		...
		Record6<String, Byte, Byte, Byte, Byte, UInteger> targetBoardInforRecord = dsl
				...
				.from(SB_BOARD_INFO_TB).where(SB_BOARD_INFO_TB.BOARD_ID.eq(targetBoardID)).forUpdate().fetchOne();
		...
		UInteger sourceGroupNo = ServerDBUtil.lockRootRecordOfBoardGroup(dsl, sourceBoardID, sourceBoardNo);
		...
		/**
		 * WARNING! 게시글 번호 오름 차순은 게시글 작성 순서이다. 이 순서로 게시글 번호를 할당하면 부모 글 부터 먼저 글 번호가
		 * 확정되는것을 보장할 수 있다. 부모 글 번호를 확정해야 이동이 가능하므로 부모 글 번호 부터 먼저 확정되는것을 보장하는 '게시글 번호 오름
		 * 차순'을 꼭 지킬것.
		 */
		Result<Record8<UInteger, UShort, UInteger, UByte, Integer, Byte, UByte, String>> boardResult = dsl
				.select(SB_BOARD_TB.BOARD_NO, SB_BOARD_TB.GROUP_SQ, SB_BOARD_TB.PARENT_NO, SB_BOARD_TB.DEPTH,
						SB_BOARD_TB.VIEW_CNT, SB_BOARD_TB.BOARD_ST, SB_BOARD_TB.NEXT_ATTACHED_FILE_SQ,
						SB_BOARD_TB.PWD_BASE64)
				.from(SB_BOARD_TB).where(SB_BOARD_TB.BOARD_ID.eq(sourceBoardID))
				.and(SB_BOARD_TB.GROUP_NO.eq(sourceGroupNo)).orderBy(SB_BOARD_TB.BOARD_NO.asc()).fetch();
		...
		for (Record8<UInteger, UShort, UInteger, UByte, Integer, Byte, UByte, String> boardRearcod : boardResult) {
			...
			try {
				...
				String errorMessage = new StringBuilder().append("이동할  ").append(e.getMessage()).toString();
				throw new RollbackServerTaskException(errorMessage);
			}
			...
			dsl.insertInto(SB_BOARD_TB).set(SB_BOARD_TB.BOARD_ID, targetBoardID).set(SB_BOARD_TB.BOARD_NO, toBoardNo)
					.set(SB_BOARD_TB.GROUP_NO, targetGroupNo).set(SB_BOARD_TB.GROUP_SQ, fromGroupSeqence)
					.set(SB_BOARD_TB.PARENT_NO, toParentNo).set(SB_BOARD_TB.DEPTH, fromDepth)
					.set(SB_BOARD_TB.VIEW_CNT, fromViewCount).set(SB_BOARD_TB.BOARD_ST, fromBoardState)
					.set(SB_BOARD_TB.NEXT_ATTACHED_FILE_SQ, fromNextAttachedFileSeq)
					.set(SB_BOARD_TB.PWD_BASE64, fromBoardPwdBase64).execute();

			dsl.update(SB_BOARD_FILELIST_TB).set(SB_BOARD_FILELIST_TB.BOARD_ID, targetBoardID)
					.set(SB_BOARD_FILELIST_TB.BOARD_NO, toBoardNo)
					.where(SB_BOARD_FILELIST_TB.BOARD_ID.eq(sourceBoardID))
					.and(SB_BOARD_FILELIST_TB.BOARD_NO.eq(fromBoardNo)).execute();

			dsl.update(SB_BOARD_HISTORY_TB).set(SB_BOARD_HISTORY_TB.BOARD_ID, targetBoardID)
					.set(SB_BOARD_HISTORY_TB.BOARD_NO, toBoardNo).where(SB_BOARD_HISTORY_TB.BOARD_ID.eq(sourceBoardID))
					.and(SB_BOARD_HISTORY_TB.BOARD_NO.eq(fromBoardNo)).execute();

			dsl.update(SB_BOARD_VOTE_TB).set(SB_BOARD_VOTE_TB.BOARD_ID, targetBoardID)
					.set(SB_BOARD_VOTE_TB.BOARD_NO, toBoardNo).where(SB_BOARD_VOTE_TB.BOARD_ID.eq(sourceBoardID))
					.and(SB_BOARD_VOTE_TB.BOARD_NO.eq(fromBoardNo)).execute();

			dsl.update(SB_MEMBER_ACTIVITY_HISTORY_TB).set(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_ID, targetBoardID)
					.set(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_NO, toBoardNo)
					.where(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_ID.eq(sourceBoardID))
					.and(SB_MEMBER_ACTIVITY_HISTORY_TB.BOARD_NO.eq(fromBoardNo)).execute();

			dsl.delete(SB_BOARD_TB).where(SB_BOARD_TB.BOARD_ID.eq(sourceBoardID))
					.and(SB_BOARD_TB.BOARD_NO.eq(fromBoardNo)).execute();
			...
			dsl.update(SB_BOARD_INFO_TB).set(SB_BOARD_INFO_TB.CNT, SB_BOARD_INFO_TB.CNT.sub(countOfBoard))
					.set(SB_BOARD_INFO_TB.TOTAL, SB_BOARD_INFO_TB.TOTAL.sub(1))
					.where(SB_BOARD_INFO_TB.BOARD_ID.eq(sourceBoardID)).execute();

			dsl.update(SB_BOARD_INFO_TB).set(SB_BOARD_INFO_TB.CNT, SB_BOARD_INFO_TB.CNT.add(countOfBoard))
			...
			Result<Record1<UByte>> attachedFileResult = dsl.select(SB_BOARD_FILELIST_TB.ATTACHED_FILE_SQ)
					.from(SB_BOARD_FILELIST_TB).where(SB_BOARD_FILELIST_TB.BOARD_ID.eq(targetBoardID))
					.and(SB_BOARD_FILELIST_TB.BOARD_NO.eq(toBoardNo)).fetch();
			...
			log.info("이동전 게시글[boardID={}, boardNo={}] ---> 이동후 게시글[boardID={}, boardNo={}]", sourceBoardID, fromBoardNo,
					targetBoardID, toBoardNo);
		}

		dsl.update(SB_BOARD_INFO_TB).set(SB_BOARD_INFO_TB.NEXT_BOARD_NO, UInteger.valueOf(targetNextBoardNo))
				.where(SB_BOARD_INFO_TB.BOARD_ID.eq(targetBoardID)).execute();

		StringBuilder siteLogStringBuilder = new StringBuilder();
		siteLogStringBuilder.append("게시글[boardID=").append(sourceBoardID).append(", boardNo=").append(sourceBoardNo)
				.append("]에 대한 '").append(targetBoardName).append("' 게시판[boardID=").append(targetBoardID)
				.append("] 이동 처리가 완료되었습니다");

		ServerDBUtil.insertSiteLog(dsl, boardMoveReq.getRequestedUserID(), siteLogStringBuilder.toString(),
