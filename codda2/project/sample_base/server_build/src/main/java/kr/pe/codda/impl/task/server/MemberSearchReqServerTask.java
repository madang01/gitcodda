...
public class MemberSearchReqServerTask extends AbstractServerTask
		implements DBAutoCommitTaskIF<MemberSearchReq, MemberSearchRes> {
	private Logger log = LoggerFactory.getLogger(MemberSearchReqServerTask.class);
	...
	public void doTask(String projectName, LoginManagerIF personalLoginManager, ToLetterCarrier toLetterCarrier,
		...
		AbstractMessage outputMessage = ServerDBUtil.execute(
				ServerCommonStaticFinalVars.DEFAULT_DBCP_NAME, this, (MemberSearchReq) inputMessage);
		toLetterCarrier.addSyncOutputMessage(outputMessage);
	}
	
	
	public MemberSearchRes doWork(final String dbcpName, MemberSearchReq memberSearchReq) throws Exception {
		MemberSearchRes outputMessage = ServerDBUtil.execute(dbcpName, this, memberSearchReq);		
		return outputMessage;
	}

	@Override
	public MemberSearchRes doWork(final DSLContext dsl, MemberSearchReq memberSearchReq) throws Exception {

		if (null == dsl) {
			throw new ParameterServerTaskException("the parameter dsl is null");
		}

		if (null == memberSearchReq) {
			throw new ParameterServerTaskException("the parameter memberSearchReq is null");
		}
		...
		try {
			...
			try {
				...
				String errorMessage = new StringBuilder().append("검색 조건인 회원 상태값[")
						.append(memberSearchReq.getMemberState()).append("]이 잘못되었습니다").toString();
			...
			if (!searchID.isEmpty()) {
				...
				for (int i = 1; i < searchIDChars.length; i++) {
					...
					if (!Character.isDigit(c) && !Character.isAlphabetic(c)) {
						String errorMessage = new StringBuilder().append("검색 조건인 아이디[").append(searchID)
			...
			try {
				fromDate = sdf.parse(new StringBuilder().append(fromDateString).append(" 00:00:00.000").toString());
			} catch (ParseException e) {
				String errorMessage = new StringBuilder().append("검색 조건인 시작일[").append(fromDateString)
			...
			try {
				toDate = sdf.parse(new StringBuilder().append(toDateString).append(" 23:59:59.999").toString());
			} catch (ParseException e) {
				String errorMessage = new StringBuilder().append("검색 조건인 종료일[").append(toDateString)
						.append("] 값이 잘못되었습니다").toString();
				throw new ParameterServerTaskException(errorMessage);
			}
			// }
		...
		ServerDBUtil.checkUserAccessRights(dsl, "회원 조회 서비스", PermissionType.ADMIN,
				memberSearchReq.getRequestedUserID());

		SelectConditionStep<Record8<String, String, UByte, Timestamp, Timestamp, Timestamp, Timestamp, Timestamp>> memberSelectConditionStep = dsl
				.select(SB_MEMBER_TB.USER_ID, SB_MEMBER_TB.NICKNAME, SB_MEMBER_TB.PWD_FAIL_CNT, SB_MEMBER_TB.REG_DT,
						SB_MEMBER_TB.LAST_NICKNAME_MOD_DT, SB_MEMBER_TB.LAST_EMAIL_MOD_DT, SB_MEMBER_TB.LAST_PWD_MOD_DT,
						SB_MEMBER_TB.LAST_STATE_MOD_DT)
				.from(SB_MEMBER_TB).where(SB_MEMBER_TB.STATE.eq(memberStateType.getValue()))
				.and(SB_MEMBER_TB.LAST_STATE_MOD_DT.between(new java.sql.Timestamp(fromDate.getTime()),
						new java.sql.Timestamp(toDate.getTime())))
				.and(SB_MEMBER_TB.ROLE.eq(MemberRoleType.MEMBER.getValue()));

		if (!searchID.isEmpty()) {
			memberSelectConditionStep = memberSelectConditionStep
					.and(SB_MEMBER_TB.USER_ID.containsIgnoreCase(searchID));
		}

		Result<Record8<String, String, UByte, Timestamp, Timestamp, Timestamp, Timestamp, Timestamp>> memberResult = memberSelectConditionStep
				.orderBy(SB_MEMBER_TB.LAST_STATE_MOD_DT.desc()).fetch();
