...
public class BoardDownloadSvl extends AbstractUserLoginServlet {
	...
	protected void performTask(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		if (null == paramAttachedFileSeq) {
			...
			String debugMessage = new StringBuilder().append(errorMessage).append(", userID=[")
					.append(accessedUserformation.getUserID()).append("], ip=[").append(req.getRemoteAddr()).append("]")
					.toString();
		...
		try {
			...
			String errorMessage = new StringBuilder("자바 short 타입 변수인 첨부 파일 순번 값[").append(paramAttachedFileSeq)
		...
		if (attachedFileSeq < 0) {
			String errorMessage = new StringBuilder("첨부 파일 순번 값[").append(paramAttachedFileSeq)
					.append("]은 0 보다 작거나 커야합니다.").toString();
		...
		if (attachedFileSeq > CommonStaticFinalVars.UNSIGNED_BYTE_MAX) {
			String errorMessage = new StringBuilder("업로드 파일 순번 값[").append(paramAttachedFileSeq).append("]은 ")
					.append(CommonStaticFinalVars.UNSIGNED_BYTE_MAX).append(" 값 보다 작거나 같아야 합니다.").toString();
		...
		// FIXME!
		{
			String debugMessage = new StringBuilder().append("inObj=").append(boardDownloadFileReq.toString())
		...
		AbstractMessage outputMessage = mainProjectConnectionPool
				.sendSyncInputMessage(ClientMessageCodecManger.getInstance(), boardDownloadFileReq);
		...
		if ((outputMessage instanceof MessageResultRes)) {
			...
			String debugMessage = new StringBuilder("입력 메시지[").append(boardDownloadFileReq.getMessageID())
		...
		if (getBrowser(req).equals("MSIE")) {
			...
			attachedFileName = URLEncoder.encode(attachedFileName, "UTF-8").replaceAll("\\+", "%20");
		...
		try {
			...
			String errorMessage = new StringBuilder().append("다운로드 파일[").append(downloadFile.getAbsolutePath())
