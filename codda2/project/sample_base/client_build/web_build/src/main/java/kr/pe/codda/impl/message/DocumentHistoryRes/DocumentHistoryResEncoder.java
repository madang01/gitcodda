...
/**
 * DocumentHistoryRes message encoder
 * @author Won Jonghoon
 *
 */
public final class DocumentHistoryResEncoder extends AbstractMessageEncoder {
	@Override
	public void encode(AbstractMessage messageObj, SingleItemEncoderIF singleItemEncoder, Object middleObjectToSend) throws Exception {
		DocumentHistoryRes documentHistoryRes = (DocumentHistoryRes)messageObj;
		encodeBody(documentHistoryRes, singleItemEncoder, middleObjectToSend);
	}


	private void encodeBody(DocumentHistoryRes documentHistoryRes, SingleItemEncoderIF singleItemEncoder, Object middleObjectToSend) throws Exception {
		...
		singleItemEncoder.putValue(pathStack.peek(), "documentNo"
			...
			, documentHistoryRes.getDocumentNo() // itemValue
			, -1 // itemSize
		...
		singleItemEncoder.putValue(pathStack.peek(), "documentSate"
			...
			, documentHistoryRes.getDocumentSate() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleObjectToSend);
		...
		singleItemEncoder.putValue(pathStack.peek(), "pageNo"
			...
			, documentHistoryRes.getPageNo() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleObjectToSend);
		...
		singleItemEncoder.putValue(pathStack.peek(), "pageSize"
			, kr.pe.codda.common.type.SingleItemType.UNSIGNED_SHORT // itemType
			, documentHistoryRes.getPageSize() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleObjectToSend);
		...
		singleItemEncoder.putValue(pathStack.peek(), "total"
			, kr.pe.codda.common.type.SingleItemType.UNSIGNED_INTEGER // itemType
			, documentHistoryRes.getTotal() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleObjectToSend);
		...
		singleItemEncoder.putValue(pathStack.peek(), "cnt"
			...
			, documentHistoryRes.getCnt() // itemValue
			, -1 // itemSize
			, null // nativeItemCharset
			, middleObjectToSend);

		java.util.List<DocumentHistoryRes.Document> document$2List = documentHistoryRes.getDocumentList();

		/** 배열 정보와 배열 크기 일치 검사 */
		if (null == document$2List) {
			/** 배열 크기 지정 방식이 간접일 경우 참조하는 변수값이 0 일 경우만 배열 값으로 null 을 허용한다. */
			if (0 != documentHistoryRes.getCnt()) {
				String errorMessage = new StringBuilder("the var document$2List is null but the value referenced by the array size[documentHistoryRes.getCnt()][").append(documentHistoryRes.getCnt()).append("] is not zero").toString();
				throw new kr.pe.codda.common.exception.BodyFormatException(errorMessage);
			}
		} else {
			int document$2ListSize = document$2List.size();
			/** 배열 값이 null 이 아닐때에는 배열 크기가 배열 정보에서 지정된 크기와 같은지 검사 */
			if (documentHistoryRes.getCnt() != document$2ListSize) {
				String errorMessage = new StringBuilder("the var document$2ListSize[").append(document$2ListSize).append("] is not same to the value referenced by the array size[documentHistoryRes.getCnt()][").append(documentHistoryRes.getCnt()).append("]").toString();
				throw new kr.pe.codda.common.exception.BodyFormatException(errorMessage);
			}
			...
			for (int i2=0; i2 < document$2ListSize; i2++) {
				...
				DocumentHistoryRes.Document document$2 = document$2List.get(i2);
