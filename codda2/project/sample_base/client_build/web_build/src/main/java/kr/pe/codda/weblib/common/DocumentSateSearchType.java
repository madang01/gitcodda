...
public enum DocumentSateSearchType {
	...
	/**
	 * 참고) 열거형의 {@link #valueOf(String)} 는 재 정의가 안되기때문에 이것을 피하기 위하여 의미 없는 파라미터 'type' 를 추가하였다. 
	 *  
	 * @param documentSateSearchTypeValue 문서 상태 유형 값
	 * @param type {@link #valueOf(String)} 와 충둘을 막기 위한 파라미터
	 * @return 파라미터 'documentSateSearchTypeValue' 와 값이 일치하는 문서 상태 유형 
	 * @throws IllegalArgumentException 파라미터 'documentStateTypeValue'(=문서 상태 유형 값) 와 일치하는 값을 갖는 문서 상태 유형이 없을 경우 던지는 예외
	 */
	public static DocumentSateSearchType valueOf(String documentSateSearchTypeValue, Class<String> type) throws IllegalArgumentException {	
		DocumentSateSearchType[] documentSateSearchTypes = DocumentSateSearchType.values();
		for (DocumentSateSearchType documentSateSearchType : documentSateSearchTypes) {
			if (String.valueOf((char)documentSateSearchType.getValue()).equals(documentSateSearchTypeValue)) {
				return documentSateSearchType;
			}
		}
