...
public class ServerIOEventController extends Thread implements
	...
	public ServerIOEventController(String projectName,
			String serverHost,
			int serverPort,
			int maxClients,
			long socketTimeOut,
	...
	public void run() {		
		log.log(Level.INFO, new StringBuilder()
				.append("ServerIOEventController::projectName[")
				.append(projectName)				
		...
		try {
				...
				try {
					for (SelectionKey selectedKey : selectedKeySet) {
							...
							if (selectedKey.isAcceptable()) {	
								...
								try {
									if (acceptedSocketChannel
										...
										log.info(new StringBuilder().append("OP_CONNECT but a connection operation is in progress on this accepted channel[")
												.append(acceptedSocketChannel.hashCode())
												.append("]").toString());
									...
									log.info(new StringBuilder()
											.append("successfully changed acceptedKey[")
											.append(acceptedKey.hashCode())
											.append("]'s accepted socket channel[")
											.append(acceptedSocketChannel.hashCode())
						...
						if (null == attachedObject) {
							...
							log.warning(new StringBuilder().append("this selectedKey2AcceptedConnectionHash map contains no mapping for the key[")
									.append(selectedKey.hashCode())
									.append("][")
			...
			log.warning(new StringBuilder().append(projectName)
					.append(" ServerIOEventController loop exit").toString());
		} catch (InterruptedException e) {
			log.warning(new StringBuilder().append(projectName)
					.append(" ServerIOEventController stop").toString());
		} catch (Exception e) {
	...
	public void registerloginUser(SelectionKey selectedKey, String loginID) {
		...
		if (selectedKey2LonginIDHash.containsKey(selectedKey)) {			
			...
			log.warning(new StringBuilder().append("the parameter selectedKey[")
					.append(selectedKey.hashCode())
		...
		if (longinID2SelectedKeyHash.containsKey(loginID)) {
			...
			log.warning(new StringBuilder().append("the parameter loginID[")
					.append(loginID)
		...
		log.info(new StringBuilder().append("login register success, selectedKey=")
				.append(selectedKey.hashCode())
				.append(", socketChannel=")
				.append(selectedKey.channel().hashCode())
				.append(", loginID=")
