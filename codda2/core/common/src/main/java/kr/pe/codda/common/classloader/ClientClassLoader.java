...
/**
 * 클라이언트용 클래스 로더, WARNING! 아직 미완성임.
 * @author Won Jonghoon
 *
 */
public class ClientClassLoader extends ClassLoader implements MessageDecoderManagerIF {
	 ...
	 */
	public ClientClassLoader(String classloaderClassPathString, String classloaderReousrcesPathString,
		...
		log.log(Level.INFO, "ClientClassLoader hashCode=[" + this.hashCode() + "] create");
	}
	 ...
	 * @return 파라미터 'messageID' 에 1:1 대응하는 클라이언트 메시지 코덱
	 * @throws DynamicClassCallException 동적 클래스 관련 처리중 에러 발생시 던지는 예외
	 */
	public MessageCodecIF getClientMessageCodec(String messageID) throws DynamicClassCallException {
		...
		if (null == messageCodec) {
			synchronized (messageCodecHash) {
				...
				if (null == messageCodec) {
					messageCodec = createNewClientMessageCodec(messageID);			
	 ...
	 * 
	 * @param messageID 메시지 식별자
	 * @return 파라미터 'messageID' 에 1:1 대응하는 신규 클라이언트 메시지 코덱
	 * @throws DynamicClassCallException 동적 클래스 관련 처리중 에러 발생시 던지는 예외
	 */
	private MessageCodecIF createNewClientMessageCodec(String messageID) throws DynamicClassCallException {
	...
	public AbstractMessageDecoder getMessageDecoder(String messageID) throws DynamicClassCallException {
		MessageCodecIF messageCodec = getClientMessageCodec(messageID);		
		return messageCodec.getMessageDecoder();
	}
	...
	protected void finalize() throws Throwable {
		...
		log.log(Level.INFO, "ClientClassLoader[" + this.hashCode() + "] destroy");
	}
}
